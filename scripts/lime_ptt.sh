...
# Called by rpidatvtouch to delay the PTT 8 seconds to allow lime calibration off-air before transmitting
...
if ((pgrep -x "limesdr_send" > /dev/null) || (pgrep -x "limesdr_dvb" > /dev/null) || (pgrep -x "dvb_t_stack" > /dev/null))
then
  # set PTT high
